...
- **Skill `logicspec-authoring`** — the DSL rules (nine step types, transition
  ...
  and reference sheets for the full grammar and every LS diagnostic code.
...
- **`/logicspec:check [path]`** — validate the workspace and fix findings by
  LS code.
- **MCP server** — registers `logicspec mcp`, exposing `list_features`,
  `get_feature`, `get_step`, `get_transitions`, `get_service_dependencies`,
  `get_events`, `validate_feature`.
