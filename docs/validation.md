...
The **whole catalog is validated at load**, independently of any feature: the complete `$ref` graph among definitions is checked (cycles → LS112, unknown targets → LS110, malformed/cross-section pointers → LS111) even when no feature references a definition, and every concrete step template is checked against its per-type shape (a bad template — unknown type, wrong field type, missing type-essential field — is reported against `definitions.yaml`, not the feature that references it). A definition-to-definition `$ref` chain is bounded: a chain deeper than 100 links is rejected as LS112 ("chain too deep") rather than being followed, so a crafted catalog can never drive resolution into a crash.
