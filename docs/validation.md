...
**Shared definitions (`$ref`).** An optional `definitions.yaml` catalog (configured as `catalogs.definitions`) holds named reusable **actors** and **step templates**. A feature actor may be `{ $ref: "definitions#/actors/<name>", …overrides }` and a step body may be `{ $ref: "definitions#/steps/<name>", …overrides }` — the map key is the actor/step id, and any local keys shallow-merge over the resolved actor/template (**local wins**). **Both actors and steps support call-site overrides**; the merged result is still validated against the strict actor/step schema, so an override key that is not a valid field is rejected as usual. These references are **expanded into concrete actors/steps before schema validation**, so graph building, every validator, the renderers and diff all operate on the fully-resolved feature; a feature with no `$ref` is byte-identical through the pipeline. Only intra-workspace `definitions#/…` references are accepted (no arbitrary file/URL refs), and the catalog path is subject to the same LS005 workspace-root containment as every other catalog.

The **whole catalog is validated at load**, independently of any feature: the complete `$ref` graph among definitions is checked (cycles → LS112, unknown targets → LS110, malformed/cross-section pointers → LS111) even when no feature references a definition, and every concrete step template is checked against its per-type shape (a bad template — unknown type, wrong field type, missing type-essential field — is reported against `definitions.yaml`, not the feature that references it). Reference expansion is bounded three ways so a crafted catalog can never drive resolution into a crash or an out-of-memory abort: a definition-to-definition `$ref` chain deeper than 100 links is rejected as LS112 ("chain too deep") rather than being followed; the total number of resolutions in one expansion is capped; and the **cumulative bytes of expanded definition content** are capped (5 MB) — one large template fanned out across thousands of `$ref` nodes stops at the budget with LS112 ("expansion output too large") *before* cloning, so it can never amplify a small workspace into gigabytes.
